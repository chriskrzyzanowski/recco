// Recco — Landing / marketing page (MVP — v2 trims)
// Changes from v1 per partner feedback:
//   • removed red dot from logo mark area (sticky nav simplified)
//   • removed "Get the app" button → hero shifts up
//   • kept "AI menu intelligence" pill + "Start guessing, start recommending"
//   • "3 seconds" → "three seconds"
//   • removed Features grid section
//   • removed final "Try Recco" CTA section (presumes app already downloaded)

function LandingScreen({ onGetStarted }) {
  return (
    <div style={{ width: '100%', height: '100%', overflowY: 'auto', background: 'var(--bone)' }}>
      {/* Sticky nav — logo only, no CTA */}
      <div style={{
      ...
      {/* Hero — moved up since we removed the duplicate Get-the-app CTA */}
      <div style={{ padding: '32px 28px 32px' }}>
        ...
        <div style={{ fontSize: 38, fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.05, marginTop: 16 }}>
          Stop guessing.<br/>Start recommending.
        </div>
        <div style={{ fontSize: 15, color: 'var(--char-2)', marginTop: 14, lineHeight: 1.5 }}>
          Point your camera at any menu. Recco reads every dish, flags allergens, and ranks the picks for your taste — in under five seconds.
        </div>

        <div style={{ marginTop: 24, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <PrimaryButton onClick={onGetStarted} icon={<Icons.Camera size={20} />}>Start scanning</PrimaryButton>
          <div style={{ fontSize: 12, color: 'var(--char-3)', textAlign: 'center' }}>Free · No account needed</div>
        </div>
      ...
      {/* How it works — copy fix: "3 seconds" → "three seconds" */}
      <div style={{ padding: '32px 28px', background: 'var(--cream)' }}>
        ...
        <div style={{ marginTop: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          {[
            ...
            ['02', 'AI reads everything', 'Ingredients parsed, allergens flagged. Roughly three seconds.', '#7A8A6F'],
            ['03', 'Your perfect pick', 'Ranked for your taste and your diet, with reasoning.', '#D9B864'],
      ...
      {/* Stats — kept; tightened to drop "24 Languages" since multi-language is cut */}
      ...
      {/* Footer — no final CTA section (presumes downloaded) */}
