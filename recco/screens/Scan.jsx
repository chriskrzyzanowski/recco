  ...
  return (
    <div style={{ width: '100%', height: '100%', background: '#0F0E0C', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Demo trigger — for prototype reviewers; surfaces the error states.
          Gated behind window.RECCO_SHOW_DEMO_ERRORS so the buttons don't appear
          in a clean user demo. Flip the flag in Recco.html to expose them. */}
      {phase === 'aim' && window.RECCO_SHOW_DEMO_ERRORS && (
...
// ─── FakeMenu — the page-shaped menu rendered inside the viewfinder ──
// Pulls the first 6 dishes from the active restaurant so the scan animation
// reads as recognizably "the place you picked," not a generic mockup.
function FakeMenu({ restaurant }) {
  const r = restaurant || {};
  const dishes = (r.dishes || []).slice(0, 6);
  return (
    <div style={{
      padding: '20px 18px', height: '100%', boxSizing: 'border-box',
      color: '#2a241c',
    }}>
      <div style={{ textAlign: 'center', borderBottom: '1px solid #c9bfae', paddingBottom: 10 }}>
        <div style={{ fontFamily: 'serif', fontSize: 18, fontWeight: 800, letterSpacing: '0.02em' }}>
          {r.name || 'Menu'}
        </div>
        {r.neighborhood && (
          <div style={{ fontSize: 8, marginTop: 3, color: '#7a6f60', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
            {r.neighborhood}
          </div>
        )}
      </div>
